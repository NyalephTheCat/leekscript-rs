...
use crate::formatter::options::{BraceStyle, FormatterOptions, IndentStyle, SemicolonStyle};
...
fn parse_format_table(t: &toml::map::Map<String, toml::Value>) -> Option<FormatterOptions> {
    ...
    Some(FormatterOptions {
        preserve_comments: t.get("preserve_comments").and_then(|v| v.as_bool()).unwrap_or(true),
        parenthesize_expressions: t.get("parenthesize_expressions").and_then(|v| v.as_bool()).unwrap_or(false),
