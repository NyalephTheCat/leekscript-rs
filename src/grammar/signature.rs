...
/// Add lexer rules for signature file: trivia, keywords, ident, punctuation.
fn add_sig_lexer(g: &mut sipha::builder::GrammarBuilder) {
    super::trivia::add_ws(g);

    g.lexer_rule("sig_kw_function", |g| sig_kw(g, Kind::KwFunction, b"function"));
    g.lexer_rule("sig_kw_class", |g| sig_kw(g, Kind::KwClass, b"class"));
    g.lexer_rule("sig_kw_extends", |g| sig_kw(g, Kind::KwExtends, b"extends"));
    g.lexer_rule("sig_kw_constructor", |g| sig_kw(g, Kind::KwConstructor, b"constructor"));
    ...
    g.lexer_rule("sig_kw_protected", |g| sig_kw(g, Kind::KwProtected, b"protected"));
    ...
    g.lexer_rule("sig_ident", |g| {
        g.choices(vec![
            Box::new(|g| { g.call("sig_kw_function"); }),
            Box::new(|g| { g.call("sig_kw_class"); }),
            Box::new(|g| { g.call("sig_kw_extends"); }),
            Box::new(|g| { g.call("sig_kw_constructor"); }),
            Box::new(|g| { g.call("sig_kw_static"); }),
            Box::new(|g| { g.call("sig_kw_public"); }),
            Box::new(|g| { g.call("sig_kw_private"); }),
            Box::new(|g| { g.call("sig_kw_protected"); }),
            Box::new(|g| { g.call("sig_kw_final"); }),
            Box::new(|g| { g.call("sig_kw_global"); }),
            Box::new(sig_ident_token),
        ]);
    });

    g.lexer_rule("sig_lparen", |g| g.token(Kind::TokParenL, |g| { g.byte(b'('); }));
    g.lexer_rule("sig_rparen", |g| g.token(Kind::TokParenR, |g| { g.byte(b')'); }));
    g.lexer_rule("sig_lbrace", |g| g.token(Kind::TokBraceL, |g| { g.byte(b'{'); }));
    g.lexer_rule("sig_rbrace", |g| g.token(Kind::TokBraceR, |g| { g.byte(b'}'); }));
    g.lexer_rule("sig_comma", |g| g.token(Kind::TokComma, |g| { g.byte(b','); }));
    g.lexer_rule("sig_colon", |g| g.token(Kind::TokColon, |g| { g.byte(b':'); }));
    g.lexer_rule("sig_arrow", |g| {
        g.choices(vec![
            Box::new(|g| g.token(Kind::TokArrow, |g| { g.literal(b"->"); })),
            Box::new(|g| g.token(Kind::TokArrow, |g| { g.literal(b"=>"); })),
        ]);
    });
    g.lexer_rule("sig_lt", |g| g.token(Kind::TokOp, |g| { g.byte(b'<'); }));
    g.lexer_rule("sig_gt", |g| g.token(Kind::TokOp, |g| { g.byte(b'>'); }));
    g.lexer_rule("sig_question", |g| g.token(Kind::TokOp, |g| { g.byte(b'?'); }));
    g.lexer_rule("sig_pipe", |g| g.token(Kind::TokOp, |g| { g.byte(b'|'); }));
}
...
fn add_sig_type_optional(g: &mut sipha::builder::GrammarBuilder) {
    g.parser_rule("sig_type_optional", |g| {
        ...
        g.optional(|g| { g.call("sig_question"); });
    });
}
...
fn add_sig_method(g: &mut sipha::builder::GrammarBuilder) {
    g.parser_rule("sig_method", |g| {
        g.node(Kind::NodeSigMethod, |g| {
            g.optional(|g| { g.call("sig_kw_static"); });
            g.optional(|g| {
                g.choices(vec![
                    Box::new(|g| { g.call("sig_kw_public"); }),
                    Box::new(|g| { g.call("sig_kw_private"); }),
...
fn add_sig_field(g: &mut sipha::builder::GrammarBuilder) {
    g.parser_rule("sig_field", |g| {
        g.node(Kind::NodeSigField, |g| {
            g.optional(|g| { g.call("sig_kw_static"); });
            g.optional(|g| { g.call("sig_kw_final"); });
            g.call("sig_type_expr");
            g.call("sig_ident");
        });
    });
...
fn add_sig_class_member(g: &mut sipha::builder::GrammarBuilder) {
    g.parser_rule("sig_class_member", |g| {
        g.choices(vec![
            Box::new(|g| { g.call("sig_method"); }),
            Box::new(|g| { g.call("sig_constructor"); }),
            Box::new(|g| { g.call("sig_field"); }),
        ]);
    });
}
...
fn add_sig_class(g: &mut sipha::builder::GrammarBuilder) {
    g.parser_rule("sig_class", |g| {
        g.node(Kind::NodeSigClass, |g| {
            ...
            g.call("sig_lbrace");
            g.zero_or_more(|g| { g.call("sig_class_member"); });
            g.call("sig_rbrace");
        });
    });
}
...
fn add_sig_item(g: &mut sipha::builder::GrammarBuilder) {
    g.parser_rule("sig_item", |g| {
        g.choices(vec![
            Box::new(|g| { g.call("sig_function"); }),
            Box::new(|g| { g.call("sig_class"); }),
            Box::new(|g| { g.call("sig_global"); }),
        ]);
    });
}
...
fn add_sig_file(g: &mut sipha::builder::GrammarBuilder) {
    g.parser_rule("sig_file", |g| {
        g.node(Kind::NodeSigFile, |g| {
            g.zero_or_more(|g| { g.call("sig_item"); });
        });
    });
}
...
mod tests {
    ...
    fn parse_example_signature_file() {
        ...
        assert!(root.children().next().is_some(), "expected at least one sig item");
    }
    ...
    fn parse_generated_stdlib_signatures() {
        ...
        for name in ["stdlib_functions.sig", "stdlib_constants.sig"] {
            ...
            let result = parse_signatures(&src).expect("parse should not error");
            let root = result.expect("parse should produce a root");
            assert!(root.children().next().is_some(), "{} should have sig items", name);
            parsed += 1;
        }
