...
impl Transformer for ExpandAssignAdd {
    fn transform_node(&mut self, node: &SyntaxNode) -> TransformResult {
        ...
        let rhs_green = parse_binary_add_rhs(&postfix_node.collect_text(), &expr_node.collect_text())
...
mod tests {
    ...
    fn transform_identity_keeps_tree() {
        ...
        assert!(sipha_diff::trees_equal(&root, &new_root), "{}", sipha_diff::format_diff(&root, &new_root));
    }
}
