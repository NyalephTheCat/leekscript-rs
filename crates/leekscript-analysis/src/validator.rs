...
use super::node_helpers::{
    class_decl_info, expr_identifier, for_in_loop_vars, function_decl_info, param_name,
    var_decl_info,
};
...
pub struct Validator<'a> {
    ...
    stack: Vec<ScopeId>,
    /// Names declared in the current scope with span of first declaration (for duplicate detection and related info).
    declared_in_scope: Vec<HashMap<String, Span>>,
...
impl<'a> Validator<'a> {
    ...
    pub fn new(store: &'a ScopeStore, scope_id_sequence: &'a [ScopeId]) -> Self {
        Self {
            store,
            stack: vec![ScopeId(0)],
            declared_in_scope: vec![HashMap::new()],
            scope_id_index: 0,
            scope_id_sequence,
            diagnostics: Vec::new(),
        }
    ...
    fn push_scope(&mut self) {
        ...
        self.stack.push(id);
        self.declared_in_scope.push(HashMap::new());
    }
    ...
    fn resolve(&self, name: &str) -> bool {
        if self.store.resolve(self.current_scope(), name).is_some() {
            return true;
        }
    ...
    fn in_main_block(&self) -> bool {
        self.stack.iter().all(|&id| {
            self.store
                .get(id)
                .is_none_or(|s| s.kind == ScopeKind::Main || s.kind == ScopeKind::Block)
        })
    }
        ...
        match kind {
                ...
                if matches!(kind, Kind::NodeForInStmt) {
                    for (name, span) in for_in_loop_vars(node) {
                        if let Some(declared) = self.declared_in_scope.last_mut() {
                            declared.entry(name).or_insert(span);
                        }
                    }
                }
            }
                    ...
                    if let Some(info) = function_decl_info(node) {
                        ...
                        if let Some(main) = self.store.get(self.store.root_id()) {
                            if let Some(existing_span) = main.get_function_span_for_arity_range(
                                ...
                                {
                                    self.diagnostics.push(
                                        AnalysisError::DuplicateFunctionName.at_with_related(
                                            info.name_span,
                                            vec![(existing_span, "first declared here")],
                                        ),
                                    );
                                }
                            }
                ...
                if let Some(info) = class_decl_info(node) {
                    if let Some(main) = self.store.get(self.store.root_id()) {
                        if let Some(first_span) = main.get_class_first_span(&info.name) {
                            if first_span != info.name_span && first_span != Span::new(0, 0) {
                                self.diagnostics.push(
                                    AnalysisError::DuplicateClassName.at_with_related(
                                        info.name_span,
            ...
            Kind::NodeVarDecl => {
                if let Some(info) = var_decl_info(node) {
                    ...
                    if let Some(declared) = self.declared_in_scope.last_mut() {
                        match declared.entry(info.name.clone()) {
                            std::collections::hash_map::Entry::Occupied(entry) => {
                                let first_span = *entry.get();
                                self.diagnostics.push(
                                    AnalysisError::VariableNameUnavailable.at_with_related(
                                        info.name_span,
                                        vec![(first_span, "first declared here")],
                                    ),
                                );
