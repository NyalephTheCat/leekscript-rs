...
pub fn reparse(
    ...
    let (_, graph) = program_built_and_graph();
    let mut engine = Engine::new().with_memo();
    sipha_reparse(&mut engine, graph, old_source.as_bytes(), old_root, edit)
}

/// Apply a text edit and reparse incrementally, or fall back to full parse.
///
/// Applies `edit` to `old_source` to obtain the new source. If `old_root` is `Some`,
/// tries [`reparse`]; on success returns the new root. On reparse failure or when
/// `old_root` is `None`, runs [`parse`](parse) on the new source and returns the result.
/// Use in LSP or editors to update the tree after a single range-based change.
#[must_use]
pub fn reparse_or_parse(
    ...
    let new_source = std::str::from_utf8(&new_source_bytes).ok()?;
    if let Some(root) = old_root {
        if let Ok(Some(new_root)) = reparse(old_source, root, edit) {
            return Some(new_root);
        }
    }
    parse(new_source).ok().and_then(std::convert::identity)
}
...
pub fn parse_error_to_diagnostics(
    ...
    let (span, message) = match parse_err {
        ...
        ParseError::BadGraph => (Span::new(0, 0), "malformed parse graph".to_string()),
    };
    vec![sipha::error::SemanticDiagnostic {
        span,
        message,
...
mod tests {
    ...
    fn parse_edge_unterminated_double_quote_string() {
        ...
        assert!(
            result.is_err(),
            "unterminated double-quote string should fail"
        );
    }
    ...
    fn parse_edge_unterminated_single_quote_string() {
        ...
        assert!(
            result.is_err(),
            "unterminated single-quote string should fail"
        );
    }
    ...
    fn parse_valid_program() {
        ...
        let node_children: Vec<_> = root
            ...
            .collect();
        assert!(
            !node_children.is_empty(),
            "root should have statement children"
        );
        assert_eq!(
    ...
    fn parse_error_to_miette_produces_report() {
        ...
        assert!(
            report.is_some(),
            "NoMatch parse error should produce a miette report"
        );
    ...
    fn assert_parse_sexp() {
        ...
        assert!(
            expected.contains("EXPR"),
            "readable kind names in S-expression"
        );
    }
    ...
    fn reparse_after_edit() {
        ...
        assert!(
            new_text.contains("2"),
            "edited content in reparsed tree: {:?}",
            new_text
        );
    }

    // binary_expr_rhs_field test lives in leekscript-analysis (uses analysis::binary_expr_rhs).
    ...
    fn assert_parse_function_decl(source: &str, test_name: &str) {
        ...
        assert!(
            !funcs.is_empty(),
            "{}: expected at least one NodeFunctionDecl in {:?}",
            test_name,
            source
        );
    }
    ...
    fn parse_function_mixed_params_fat_arrow_return() {
        assert_parse_function_decl(
            "function a(integer b, c) => void {}",
            "mixed params, => void",
        );
    }
    ...
    fn parse_function_typed_params_arrow_return() {
        assert_parse_function_decl(
            "function a(integer x, integer y) -> integer {}",
            "typed params, -> integer",
        );
    }
    ...
    fn parse_program_with_function_type_two_args_return() {
        ...
        let root = parse(source)
            .unwrap()
            .expect("Function<integer, integer => void>");
        let type_exprs = root.find_all_nodes(Kind::NodeTypeExpr.into_syntax_kind());
        assert!(
            !type_exprs.is_empty(),
            "expected NodeTypeExpr for Function<...> type"
        );
    }
    ...
    fn parse_program_with_function_type_zero_params() {
        ...
        assert!(
            !type_exprs.is_empty(),
            "expected NodeTypeExpr for Function< => void>"
        );
    }
    ...
    fn parse_program_with_function_type_one_param() {
        ...
        assert!(
            !type_exprs.is_empty(),
            "expected NodeTypeExpr for Function<integer => void>"
        );
    }
    ...
    fn parse_program_with_function_type_three_params() {
        ...
        let root = parse(source)
            .unwrap()
            .expect("Function<integer, string, real => boolean>");
        let type_exprs = root.find_all_nodes(Kind::NodeTypeExpr.into_syntax_kind());
        assert!(
            !type_exprs.is_empty(),
            "expected NodeTypeExpr for Function<...>"
        );
    }

    // ─── Anonymous function (function(a, b) { ... }) ───────────────────────────

    #[test]
    fn parse_anonymous_function() {
        let source = "var sum = function(a, b) { return a + b; };";
        let root = parse(source).unwrap().expect("anonymous function");
        let anon_fns = root.find_all_nodes(Kind::NodeAnonFn.into_syntax_kind());
        assert!(
            !anon_fns.is_empty(),
            "expected NodeAnonFn for function(a, b) {{ ... }}"
        );
    }
}
